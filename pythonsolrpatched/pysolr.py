...
... 	{'id': 2,  'name': 'document 2', 'text': u'Ð’Ð»Ð°Ð´Ð¸Ð¼Ð¸Ñ€ ÐœÐ°ÑÐºoÐ²ÑÐºÐ¸Ð¹'},
...
>>> results = conn.search(u'Ð’Ð»Ð°Ð´Ð¸Ð¼Ð¸Ñ€')
>>> len(results)
1
...
class Solr(object):
	...
	def _select(self, params):
		...
		path = '%s/%s/?%s' % (self.path, self.reqhandlerstring, urlencode(params, True))
	...
	def _select_post(self,params):
		...
		params['q'] = self._encode_q(params['q'])
		params['wt'] = 'json' 
		path = '%s/%s?' % (self.path, self.reqhandlerstring)
	...
	def search(self, q, **kwargs):
		...
		try:
			using = params['usingreqhandler']
			self.reqhandlerstring='clustering'
		except:
			self.reqhandlerstring='select'
...
#######################################################
  	...
  	# medialab patch to send raw xml docs to solr
  	def addrawxml(self, xmlcontent):
		path = '%s/dataimport?command=full-import&clean=false' % self.path
		return self._send_request('POST', path, xmlcontent, {'Content-type': 'text/xml'})
